...
Page({
    ...
    onGoStudentList() {
        ...
        wx.navigateTo({
            url: '../selectStudent/index?contactsList=' + contactsList,
        })
    },
    ...
    _submitOrder() {
        let buyMoney = this.data.total,  //价格
            ...
            lmId = this.data.currentContacts.id;
        let { type, id } = this.data.card;
        let { activateDate, isChecked, discountTotal } = this.data;
        wx.showLoading()
        orderModel.submitCardOrder({
            buyMoney,
            money: discountTotal,  //支付金额  
            activateDate: activateDate || 0 ,  //激活天数    
            cpId,
            id,
            lmId,         //联系人ID
            type,         //卡类型
